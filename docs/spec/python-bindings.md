...
- `dag.literal(<datum>)` creates a `DML_NODE_LITERAL` for datum values.
...
- `dag.call(...)` creates `DML_NODE_CALL` and returns an `ExecutionHandle`.
- `wait()` yields a DAG id on success.
- Success writes `DML_REC_EXEC(status=OK, result_value=<dag_id>)`.
- Failure writes `DML_REC_EXEC(status=ERROR, error_value=<datum_id>)` and raises
  Python exception.
